...
type ColorScheme = 'default' | 'blue' | 'green' | 'red' | 'purple'
...
const fading = ref(false)
const showMap = ref(true)
const mapColor = ref<ColorScheme>('default') // Typed as ColorScheme

const colorFilters: Record<ColorScheme, string> = {
  default: '',
  blue: 'hue-rotate(180deg) saturate(2)',
  green: 'hue-rotate(90deg) saturate(1.5)',
  red: 'hue-rotate(0deg) saturate(2) brightness(1.1)',
...
const changeColor = (color: ColorScheme) => {
...
<template>
  <nav>
    ...
    <button 
      v-for="color in Object.keys(colorFilters) as ColorScheme[]" 
      :key="color"
      @click="changeColor(color)"
      class="px-3 py-1 rounded text-white"
      :class="{
        'bg-blue-600': color === 'default',
      ...
      {{ color }}
        ...
        <template v-if="showMap">
          <img
            ...
            alt="County Map"
            class="w-full max-w-4xl mx-auto rounded-lg transition-filter duration-300"
            :style="{ filter: colorFilters[mapColor] }"
          />
          <p class="text-white text-md mt-2 mb-4 italic">
            Click a county to view their public golf courses
          </p>
